...
标准 G/A/S 字段没有别名。K-file NamelistBlock/变量按 Fortran 大小写不敏感。解析器内部
对象、原始布局、计数和其它实现辅助状态都不是最终 Python API；调试器中暂时看到的内部
属性也不应依赖。A-file 的公开 `header`/`footer` 文本是例外，属于正式接口。
